...
<body>
	...
	<table border="1">
		<tr><th>スレッド一覧</th><th>タイトル</th><th>名前</th><th>内容</th></tr>
		<c:forEach var="prof" items="${users}">
			<tr><td><a href="threadnew?tno=${prof.threadno}">${prof.threadno}</a></td>
			<td>${prof.title}</td><td>${prof.name}</td><td>${prof.contents}</td>
			<td><form method='post' action='indexnew'>
			<button name="report" value="${prof.threadno}">報告</button>
			</form></td><td>スレッド番号${prof.threadno}</td></tr>
		</c:forEach>
	</table>
